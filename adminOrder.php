            ...
            <div class="box-container" id="orderList">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        $Reservation_ID = $row['Customer_ID'];
                        echo'<tr>
                                <td class="user-id">'.$row['Customer_ID'].'</td>
                                ...
                                <td>'.$row['Status'].'</td> 
                                <td>
                                  <a class="remove-row-button" href="DeletionQueries.php?act=DeleteUser&Customer_ID=' . urlencode($row['Customer_ID']) . '" onclick="return confirm(\'Are you sure you want to delete this order?\');" class="icon-button">
